...
public class HDRProcessor {
	...
	private static class ResponseFunction {
		 ...
		 * @param context We pass the context, so this inner class can be made static.
	...
	private void processHDRCore(List<Bitmap> bitmaps) {
		...
		if( use_renderscript ) {
			...
			for(int i=0;i<n_threads;i++) {
				...
				threads[i] = new HDRWriterThread(y_start, y_stop, bitmaps, response_functions);
			}
			// start threads
			if( MyDebug.LOG )
